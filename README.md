# Go API client for dofusdude-go
...
```golang
import dofusdude-go "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
```golang
ctx := context.WithValue(context.Background(), dofusdude-go.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), dofusdude-go.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), dofusdude-go.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), dofusdude-go.ContextOperationServerVariables, map[string]map[string]string{
